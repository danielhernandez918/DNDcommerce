		...
		<div class="d-flex col-12">
			<table class="table subTable">
				...
				<c:forEach var="c" items="${cart}">
					<c:set var="sub" value="${sub + c.item.price*c.quantity}"/>
							<tr>
								<td>
								...
								<td>${c.item.price}</td>
								<td>${c.quantity}</td>
								<c:set var="subtotal" value="${c.quantity * c.item.price}" />
								<td>${subtotal}</td>
								<td class="">
			...
			<div class="col-3" >
				<div class="d-flex">
					...
					<p class="sub2 col-6">
						$${sub}<br>
						<c:set var="tax" value="${sub*.09}" />
						<fmt:setLocale value = "en_US"/>
						<fmt:formatNumber value = "${tax}" type = "currency"/>
						<br>
						<c:set var="grand" value="${sub+tax}" />
						<fmt:setLocale value = "en_US"/>
		         	...
		         	<form:form action='/charge' method='POST' modelAttribute="order" id='checkout-form'>
    						<form:hidden value='${sub}' path='subtotal' />
